...
async function migrateJsonToDb() {
  ...
  try {
    ...
    if (error.code !== 'ENOENT') {
      console.error('Migration JSON -> SQLite échouée :', error);
    }
  }
}
...
app.patch('/api/admin/messages/:id/read', requireAdmin, async (req, res) => {
  ...
  const result = await dbRun('UPDATE messages SET status = ? WHERE id = ?', ['read', id]);
  if (result.changes === 0) {
    return res.status(404).json({ error: 'Message introuvable.' });
  }

  const [message] = await dbAll('SELECT * FROM messages WHERE id = ?', [id]);
